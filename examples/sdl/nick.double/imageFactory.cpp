...
ImageFactory* ImageFactory::getInstance() {
    static ImageFactory *instance = new ImageFactory;   //meyers singleton
    return instance;
}
...
ImageFactory::~ImageFactory() {
    ...
    std::map<std::string, SDL_Texture*>::iterator texptr = textures.begin();
    while ( texptr != textures.end() ) {
        SDL_DestroyTexture(texptr->second);
        ++texptr;
    }
    std::map<std::string, Image*>::iterator imgptr = images.begin();
    while ( imgptr != images.end() ) {
        std::cout << "deleting " << imgptr->first << std::endl;
        delete imgptr->second;
        ++imgptr;
    }
